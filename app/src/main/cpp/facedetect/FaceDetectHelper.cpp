        ...
        if (mDetectFaceCallback != NULL) {
            ...
            if (pFaceInfo.face_count > 0) {
                ...
                for (int i = 0; i < len; ++i) {
                    ...
                    LOGD("byted_effect_face_detect face info rect left : %d - %d", i, item.rect.left);
                    LOGD("byted_effect_face_detect face info rect right : %d - %d", i, item.rect.right);
                    LOGD("byted_effect_face_detect face info rect top : %d - %d", i, item.rect.top);
                    LOGD("byted_effect_face_detect face info rect bottom : %d - %d", i, item.rect.bottom);
                    if (item.action > 0) {
                        //TODO: add face rect point
                        mDetectFaceCallback(item.action,item.rect.left,item.rect.right,item.rect.top,item.rect.bottom);
                    }
                }
            }
        }
